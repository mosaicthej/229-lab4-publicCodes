# combining timerEndsByQ.s and readNum.s
# read the input first, then do a timer countdown.
# kernal part will use timerEndsByQ as framework
...
noKeyText: .asciiz "no key pressed, check timer\n"
strPrompt: .asciiz "seconds = "
strNewline: .asciiz "\n"
...
beqz    $s1, quitInKernal   # if time is zero, quit
addi    $s1, $s1, -1     # minus $s1 by 1
...
keyboard:
    ...
    j       quitInKernal    # if it is `q`, end the program
...
.globl __start
__start:
    # getting prompt displayed,
    # using code from `polling.s`
    prompt: 
    la      $t0, strPrompt
    promptLoop:
        lb      $t1, 0($t0)
    ...
    # before set up timer, get the seconds from user    
    readNum:    
        lui     $t0, 0xffff
    readNumloop:
        lw	    $t1, 0($t0)	        # keyboard control
        andi	$t1, $t1, 0x0001    
        beq	    $t1, $0, readNumloop   # wait when 'ready'
